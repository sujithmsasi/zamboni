...
> REALITY: R10.2's framing conflicts with §5's framing of the same file —
> R10.2 calls "vacuum 12-gap-type hardening" an org-EXCLUSIVE feature not in
> this repo, while §5 describes the personal repo's `vacuum.py` AS IF it
> already had "12 gap types, partition-aware" logic. It does not (5 named
> gaps: 1,2,3,9,10; no partition-type awareness; single bare VACUUM per
> table). R10.2's framing matches what Phase 0 actually found — the 12-gap
> version is genuinely absent here. Treat §5's "12 gap types" line as a
> copy-paste error carried over from the org changelog, not evidence that
> the feature is secretly present. See the Conflict List below.
- **Personal repo commit history (Phase 0 audit)**: `git log --all` on this
  repo shows only feature/fix commits (gates, Control-M, table registration,
  nonprod lifecycle, etc.) — no prior `.claude/CLAUDE.md`, `decisions.md`,
  `components.md`, or `context_hints.md` were ever committed. Phase 0's
  brief ("~107 commits landed AFTER the memory files were written") assumes
  a prior version of these files that does not exist in this repo's history.
  Treated as a first-write, not a refresh — flagged, not silently patched
  over.
...
2. **"12 gap types" vacuum.py — contradictory framing across documents** —
   ...
   "12 gap types, partition-type-aware" logic; contracts §10/R10.2 and
   ORG_DROP.md describe that exact feature as org-EXCLUSIVE and absent here.
   Phase 0 confirms the ORG_DROP/R10.2 framing is correct for this repo —
