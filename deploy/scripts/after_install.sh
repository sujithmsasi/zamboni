...
# ── 2b. venv for zamboni-api.service (Phase 6) ────────────────────────────────
...
# the system-wide install above -- keeps the API service's dependency set
# (fastapi, uvicorn, python-multipart, httpx) isolated from anything else
# installed system-wide. Idempotent: skips creation if the venv already
# exists, always re-syncs requirements.
...
# ── 5. Control-plane data directory ──────────────────────────────────────────
...
# ── 6. Control-plane DB schema init/migrate ──────────────────────────────────
...
# ── 7. zamboni-control-plane-{sync,backup,integrity} systemd services ───────
