...
exist in the target account. Six steps end to end: AWS profile → both env
files (`.env.aws_local` and `.env` — yes, both matter, the guide explains
why) → dependencies → smoke test (with a description of every check) →
start the app.
